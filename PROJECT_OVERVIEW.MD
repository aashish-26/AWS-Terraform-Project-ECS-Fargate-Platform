...
- **Azure-first, container-based app platform**
  - App runs in a Docker container on **Azure App Service (Linux)**.
  - Images are built from this repo and pushed to **Azure Container Registry (ACR)** via GitHub Actions.
- **Environment layout**
...
## Q4: Why did you migrate away from AWS to Azure?

**Recruiter-style question**  
“You started on AWS. Why did you suddenly switch to Azure?”
...
The AWS design remains in the repo as a **reference architecture**, but the working, demonstrable platform is **Azure-first**.  
This migration shows **adaptability and cross-cloud proficiency**, which matters more than being tied to one provider.
...
- The project history includes:
  ...
  - IAM and trust policy problems.
  - Provider version and deprecation changes (for example, `azurerm_app_service_plan` → `azurerm_service_plan`).
- It shows the ability to **diagnose root causes** and adjust design, not just fix syntax.
...
> **“I built a full Terraform-based production-style infrastructure on Azure using modular IaC, remote state, policy enforcement, containers on App Service, and secure cloud architecture. I started on AWS but migrated to Azure due to hard account restrictions, which forced me to handle a real cross-cloud migration. I used AI as an accelerator but made and validated all key design, security, and CI/CD decisions myself. The project reflects how real companies design, secure, and automate cloud infrastructure end-to-end.”**
